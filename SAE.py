...
def Net_model(lr=0.001,decay=1e-6,momentum=0.5):
    model = Sequential()
    model.add(Dense(200, input_dim=15,W_regularizer=l2(0.000)))
...
def get_fe(n,j):
    return [numpy.mean(f_out[i*num + j*stride: (i+1)*num+j*stride, n]) for i in xrange(nb_classes)]
    
def accuracy(j):
    return [numpy.mean(numpy.equal(test_set_y[i*num + j*stride: (i+1)*num+j*stride],classes[i*num + j*stride: (i+1)*num+j*stride])) for i in xrange(nb_classes)]

if __name__ == '__main__':
    train_set_x = numpy.float64(sio.loadmat(dataset, appendmat=False)['datasets20'][0,0])
    train_set_y = numpy.int64(sio.loadmat(dataset, appendmat=False)['datasets20'][0,1])-1
    train_set_y.shape = (max(train_set_y.shape),)
    
    test_set_x = numpy.float64(sio.loadmat(dataset, appendmat=False)['datasets20'][1,0])
    test_set_y = numpy.int64(sio.loadmat(dataset, appendmat=False)['datasets20'][1,1])-1
